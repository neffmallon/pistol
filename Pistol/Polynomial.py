...
class Polynomial:
    ...
    def substitute(self,other):
        return Polynomial(substitute(self.plist,plist(other)))
...
class Rational:
    ...
    def __neg__(self): return Rational(-self.num,self.denom)
    def __pow__(self,e): return Rational(self.num**e,self.denom**e)
    def __repr__(self): return '(%s)/(%s)' % (self.num,self.denom)
    def __call__(self,x1): return self.num(x1)/self.denom(x1)

    def derivative(self):
        num = self.num.derivative()*self.denom \
              + self.denom.derivative()*self.num
        denom = self.denom**2
        return Rational(num,denom)
...
def substitute(p1,p2):
    """\
    Substitute p2 for the occurances of x in p1. That is, if
    p1 = x^2-1 and p2 = x-1, this routine will return
    (x-1)^2-1 = x^2-2x
    """
    new = [0]
    for i in range(len(p1)):
        # This is new += p1[i]*power(p2,i):
        new = add(new,mult_const(power(p2,i),p1[i]))
    return new
...
def parse_string(str=None):
    ...
    or
    x**2 - 1
    One mistake it makes is that it interprets x2 as x^2, but that
    may be a feature instead of a bug, since that's often what is
    intended.
    """
...
def test():
    ...
    # Test the rational polynomials
    r = Rational(Polynomial('x^2'),Polynomial('x^3+1'))
    print r,r.derivative()
...
if __name__ == '__main__': niklasson()
