...
"""\
NAME
    FMM.py - Implementation of the fast multipole method
...
class FMMCell:
    ...
    def set_center(self):
        ...
        self.zc = (self.zmax+self.zmin)/2.
        self.xyzc = array((self.xc,self.yc,self.zc))
        return
    ...
    def divide_points(self):
        ...
        for point in self.points:
            ...
            if xyz[0] < self.xc:
                if xyz[1] < self.yc:
                    if xyz[2] < self.zc: xmymzm.append(point)
                    else:                xmymzp.append(point)
                else:
                    if xyz[2] < self.zc: xmypzm.append(point)
                    else:                xmypzp.append(point)
            else:
                if xyz[1] < self.yc:
                    if xyz[2] < self.zc: xpymzm.append(point)
                    else:                xpymzp.append(point)
                else:
                    if xyz[2] < self.zc: xpypzm.append(point)
                    else:                xpypzp.append(point)
        self.children = [
            FMMCell(xmymzm,
                    (self.xmin,self.xc,self.ymin,self.yc,self.zmin,self.zc)),
            FMMCell(xmymzp,
                    (self.xmin,self.xc,self.ymin,self.yc,self.zc,self.zmax)),
            FMMCell(xmypzm,
                    (self.xmin,self.xc,self.yc,self.ymax,self.zmin,self.zc)),
            FMMCell(xmypzp,
                    (self.xmin,self.xc,self.yc,self.ymax,self.zc,self.zmax)),
            FMMCell(xpymzm,
                    (self.xc,self.xmax,self.ymin,self.yc,self.zmin,self.zc)),
            FMMCell(xpymzp,
                    (self.xc,self.xmax,self.ymin,self.yc,self.zc,self.zmax)),
            FMMCell(xpypzm,
                    (self.xc,self.xmax,self.yc,self.ymax,self.zmin,zelf.zc)),
            FMMCell(xpypzp,
                    (self.xc,self.xmax,self.yc,self.ymax,self.zc,self.zmax))
            ]
        self.points = [] # delete points, since they're all distributed
        return
