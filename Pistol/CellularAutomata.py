...
def all():
    "Run all CA possibilities"
    height=width=125
    for dorandom in [1]:
        for rulenum in range(256):
            if dorandom: fname = "car_%03d_3.png" % rulenum
            else: fname = "ca_%03d_3.png" % rulenum
            print "Rendering ",fname
            data = ca_data(height,width,dorandom,rulenum)
            pil_render(data,height,width,fname)
    return

def main():
    opts,args = getopt.getopt(sys.argv[1:],'h:w:rR:a')
    height = 500
    width = 500
    dorandom = 0
    rule = 22
...
if __name__ == '__main__':
    ...
    #profile.run('main()','caprof.dat')
    #prof = pstats.Stats('caprof.dat')
    #prof.strip_dirs().sort_stats('time').print_stats(20)
